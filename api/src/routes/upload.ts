...
router.post('/image', authenticateToken, upload.single('image'), async (req: AuthRequest, res) => {
  try {
    ...
    // 上传到OSS
    const result = await uploadToOSS(req.file.buffer, req.file.originalname, 'images')
...
router.post('/document', authenticateToken, upload.single('document'), async (req: AuthRequest, res) => {
  try {
    ...
    // 上传到OSS
    const result = await uploadToOSS(req.file.buffer, req.file.originalname, 'documents')
...
router.post('/avatar', authenticateToken, upload.single('avatar'), async (req: AuthRequest, res) => {
  try {
    ...
    // 上传到OSS
    const result = await uploadToOSS(req.file.buffer, req.file.originalname, 'avatars')
