...
exports.syncUsersToFeishu = syncUsersToFeishu;
exports.syncDailyStatsToFeishu = syncDailyStatsToFeishu;
exports.syncAllToFeishu = syncAllToFeishu;
...
async function syncUsersToFeishu() {
    ...
    try {
        ...
        const userStats = await Promise.all(users.map(async (user) => {
            const messageCount = await index_1.prisma.message.count({
                where: {
                    conversation: {
                        userId: user.id
                    }
        ...
        for (let i = 0; i < userStats.length; i += batchSize) {
            ...
            await axios_1.default.post(`https://open.feishu.cn/open-apis/bitable/v1/apps/${tableAppToken}/tables/${userTableId}/records/batch_create`, {
                records: batch
            }, {
                headers: {
 ...
 * 同步每日统计数据到飞书
 */
async function syncDailyStatsToFeishu() {
    const tableAppToken = process.env.FEISHU_TABLE_APP_TOKEN;
    ...
    try {
        ...
        const [totalUsers, totalConversations, totalMessages, totalDocuments, todayNewUsers, todayConversations, todayMessages] = await Promise.all([
        ...
        // 计算今日活跃用户（有会话记录的用户）
        const todayActiveSessions = await index_1.prisma.userSession.groupBy({
            by: ['userId'],
            where: {
                startTime: {
                    gte: today
        ...
        // 插入今日统计
        await axios_1.default.post(`https://open.feishu.cn/open-apis/bitable/v1/apps/${tableAppToken}/tables/${dailyStatsTableId}/records`, {
            fields: {
