...
const feishuSync_1 = require("../services/feishuSync");
...
router.get('/trends', async (req, res) => {
    try {
        ...
        dates.forEach((dateStr, index) => {
            ...
            // 1. 累计用户数 - 截至当天的总用户数
            const cumulativeCount = allUsers.filter(user => new Date(user.createdAt) <= nextDate).length;
            cumulativeUsers.push(cumulativeCount);
            // 2. 活跃用户数 - 当天有对话的用户数
            const activeUserIds = new Set(conversations
                .filter(conv => {
...
router.get('/users', async (req, res) => {
    try {
        ...
        const usersWithSessions = await Promise.all(users.map(async (user) => {
            const sessions = await (index_1.prisma.userSession ? index_1.prisma.userSession.findMany({
                where: { userId: user.id }
            }) : Promise.resolve([]));
            const sessionCount = sessions.length;
            const totalActiveDuration = sessions.reduce((sum, s) => sum + (s.duration || 0), 0);
            return {
                id: user.id,
...
router.get('/conversations', async (req, res) => {
    try {
        ...
        const [conversations, total] = await Promise.all([
            ...
            index_1.prisma.conversation.count({ where })
        ]);
        res.json({
...
router.get('/conversations/:id', async (req, res) => {
    try {
        ...
        // 为每个有图片的消息生成预签名URL
        const messagesWithImages = await Promise.all(conversation.messages.map(async (msg) => {
            let imageUrl = null;
            if (msg.imageOssKey) {
                try {
                    // 生成预签名URL，有效期1小时
...
router.get('/knowledge-base/usage', async (req, res) => {
    try {
        ...
        const usageData = await Promise.all(users.map(async (user) => {
            const chunkCount = await index_1.prisma.kbChunk.count({
                where: {
                    document: {
                        userId: user.id
                    }
            ...
            const totalSize = user.documents.reduce((sum, doc) => sum + Number(doc.fileSize || 0), 0);
            const lastUpload = user.documents.length > 0
                ? user.documents.sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime())[0].createdAt
                : null;
            return {
                id: user.id,
...
router.post('/feishu/sync', async (req, res) => {
    try {
        ...
        const result = await (0, feishuSync_1.syncAllToFeishu)();
        if (result.success) {
