...
async function parseAndStoreDocument(documentId, documentUrl) {
    try {
        ...
        if (documentUrl.startsWith('http')) {
            const response = await axios_1.default.get(documentUrl, {
                responseType: 'text',
                timeout: 30000
            });
        ...
        // 批量存储切片到数据库
        const chunkPromises = chunks.map((chunk, index) => prisma.kbChunk.create({
            data: {
                docId: documentId,
                content: chunk,
                seq: index
 ...
 * 搜索相关的文档切片
 */
async function searchDocumentChunks(query, categoryId, userId, limit = 5) {
    try {
        // 构建查询条件
        const whereClause = {};
